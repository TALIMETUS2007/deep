...
const translations = {
  en: {
    ...
    "testimonials.confirmDelete": "Delete this post? This can't be undone.",
    "community.confirmDelete": "Delete this photo and its comments? This can't be undone.",
  ...
  fr: {
    ...
    "testimonials.confirmDelete": "Supprimer cette publication\u00A0? Cette action est irr\u00E9versible.",
    "community.confirmDelete": "Supprimer cette photo et ses commentaires\u00A0? Cette action est irr\u00E9versible.",
...
/* ---------------------------------------------------------
   1B. FIREBASE — real accounts (email/password + Google) and
   Firestore as the database for testimonials + community photos.
   Requires firebaseConfig to be set in index.html (see the
   comment there) and Authentication + Firestore enabled in the
   Firebase console — see the deployment guide for exact steps.
--------------------------------------------------------- */
...
function playNotificationSound() {
  try {
    ...
    // (e.g. before any user interaction on the page).
  }
}
...
function updateAuthUI() {
  ...
  const mobileUserPill = document.getElementById("mobileUserPill");
  ...
  if (navLoginBtn) navLoginBtn.style.display = signedIn ? "none" : "inline-flex";
  if (navUserPill) navUserPill.style.display = signedIn ? "flex" : "none";
  if (navUserName) navUserName.textContent = name;
  if (navUserInitials) navUserInitials.textContent = initialsFor(name);

  if (mobileLoginBtn) mobileLoginBtn.style.display = signedIn ? "none" : "inline-flex";
  if (mobileUserPill) mobileUserPill.style.display = signedIn ? "flex" : "none";
  if (mobileUserName) mobileUserName.textContent = name;
  if (mobileUserInitials) mobileUserInitials.textContent = initialsFor(name);
...
function setupFirebaseAuth() {
  if (!auth) {
    console.warn("Firebase isn't configured yet — see the comment in index.html near firebaseConfig.");
    return;
  }
  ...
  [document.getElementById("navLogoutBtn"), document.getElementById("mobileLogoutBtn")].forEach(btn => {
    if (!btn) return;
...
const regions = [
  ...
  { tag: "Bafoussam", lat: 5.4737, lon: 10.4176, img: WM + "Chefferie%20Bafoussam%20-%20Architecture.jpg?width=700",
...
/* ---------------------------------------------------------
   4. TOAST NOTIFICATIONS
--------------------------------------------------------- */
function showToast({ icon = "fa-bell", title, body, silent = false }) {
...
function setupAuthModal() {
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!auth) { showAuthError(new Error("Firebase isn't configured yet — see index.html.")); return; }
    ...
    try {
      ...
      closeAuthModal();
      form.reset();
      showToast({ icon: "fa-circle-check", title: t("auth.welcomeToast"), body: t("auth.welcomeToastBody") });
    } catch (err) {
      showAuthError(err);
    } finally {
  ...
  // once per browser session, and only if not already signed in.
  const alreadyPrompted = sessionStorage.getItem("deep_auth_prompted");
  if (!alreadyPrompted) {
    setTimeout(() => {
      if (!getCurrentUser() && !document.getElementById("authOverlay").classList.contains("open")) {
...
/* ---------------------------------------------------------
   8. SUGGESTION BOX ("What people are saying") — Firestore
--------------------------------------------------------- */
let testimonialsCache = [];

async function fetchCustomTestimonials() {
  if (!db) return [];
  try {
    const snap = await db.collection("testimonials").orderBy("createdAt", "desc").limit(30).get();
...
async function renderCustomTestimonials() {
  ...
  testimonialsCache.forEach(item => {
    ...
    const canDelete = user && item.ownerUid === user.uid;
    card.innerHTML = `
      ${canDelete ? `<button class="card-delete-btn" data-delete-testimonial="${item.id}" aria-label="Delete"><i class="fa-solid fa-trash"></i></button>` : ""}
...
function setupSuggestionBox() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!text) return;
    const user = getCurrentUser();
    if (!user || !db) { openAuthModal("signup"); return; }
    ...
    try {
      ...
      textarea.value = "";
      await renderCustomTestimonials();
      showToast({ icon: "fa-circle-check", title: t("suggestion.posted"), body: "" });
    } catch (err) {
      console.error(err);
      showToast({ icon: "fa-triangle-exclamation", title: "Error", body: err.message });
    }
  });

  // Delegated delete handler for own testimonials.
  grid.addEventListener("click", async (e) => {
    const btn = e.target.closest("[data-delete-testimonial]");
    if (!btn) return;
    const id = btn.dataset.deleteTestimonial;
...
/* ---------------------------------------------------------
   9. COMMUNITY GALLERY (seed photos + uploads + comments) — Firestore
--------------------------------------------------------- */
...
async function fetchUploadedPhotos() {
  if (!db) return [];
  try {
    const snap = await db.collection("communityPhotos").orderBy("createdAt", "desc").limit(30).get();
    return snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  } catch (err) {
    console.error("Could not load community photos:", err);
    return [];
  }
}

async function fetchSeedExtraComments(seedId) {
  if (!db) return [];
  try {
    const doc = await db.collection("seedComments").doc(seedId).get();
...
async function addSeedComment(seedId, comment) {
  await db.collection("seedComments").doc(seedId).set(
    { comments: firebase.firestore.FieldValue.arrayUnion(comment) },
    { merge: true }
  );
}

async function addUploadedPhotoComment(photoId, comment) {
...
// turned into a base64 string, so it stays comfortably under
...
function communityCardHTML(item, index, extraSeedComments) {
  ...
  const displayDate = isSeed
    ...
    : (item.createdAt && item.createdAt.toDate
        ? item.createdAt.toDate().toLocaleDateString(currentLang === "fr" ? "fr-FR" : "en-US", { year: "numeric", month: "short", day: "numeric" })
        : "");
  const initials = isSeed ? "DA" : initialsFor(item.userName);
  const imageSrc = isSeed ? item.imageUrl : item.imageData;
  const altText = isSeed ? item.alt : `Photo shared by ${item.userName}`;
  const canDelete = !isSeed && user && item.ownerUid === user.uid;
  ...
  const seedComments = isSeed
    ? item.defaultComments.map(c => `<div class="community-comment"><strong>${c.name}:</strong>${c[currentLang]}</div>`).join("")
      + (extraSeedComments || []).map(c => `<div class="community-comment"><strong>${c.name}:</strong>${c.text}</div>`).join("")
  ...
  return `
    <article class="community-card reveal in-view" data-photo-id="${item.id}">
      <div class="community-photo">
        ...
        ${canDelete ? `<button class="card-delete-btn" data-delete-photo="${item.id}" aria-label="Delete"><i class="fa-solid fa-trash"></i></button>` : ""}
      </div>
      ...
      ${captionHTML}
      <div class="reaction-bar" data-item-id="${item.id}">${reactionBarHTML(item.id)}</div>
      <div class="community-comments" id="comments-${item.id}">${seedComments}</div>
      <form class="community-comment-form" data-photo-id="${item.id}" data-is-seed="${isSeed}">
...
async function renderCommunityGallery() {
  ...
  const extraCommentsEntries = await Promise.all(
    SEED_COMMUNITY_PHOTOS.map(sp => fetchSeedExtraComments(sp.id))
  );
  const extraCommentsMap = {};
  SEED_COMMUNITY_PHOTOS.forEach((sp, i) => { extraCommentsMap[sp.id] = extraCommentsEntries[i]; });

  const combined = [...SEED_COMMUNITY_PHOTOS, ...uploaded];
  grid.innerHTML = combined.map((item, i) => communityCardHTML(item, i, extraCommentsMap[item.id])).join("");
...
function updateCommunityGalleryAccessState() {
  ...
  if (!uploadBox) return;
  const user = getCurrentUser();
  uploadBox.style.display = user ? "block" : "none";
...
function setupCommunityGallery() {
  ...
  fileInput.addEventListener("change", async (e) => {
    ...
    try {
      ...
      console.error(err);
      showToast({ icon: "fa-triangle-exclamation", title: "Upload failed", body: err.message });
    }
    fileInput.value = "";
  });

  // Delegated submit handler for comment forms.
  grid.addEventListener("submit", async (e) => {
    ...
    const isSeed = form.dataset.isSeed === "true";
    const comment = { name: user.displayName || user.email.split("@")[0], text, date: new Date().toISOString() };

    // Append immediately to the DOM so it "immediately appears,"
    ...
    try {
      if (isSeed) await addSeedComment(photoId, comment);
      else await addUploadedPhotoComment(photoId, comment);
    } catch (err) {
      console.error(err);
    }
  });

  // Delegated delete handler for own uploaded photos.
  grid.addEventListener("click", async (e) => {
    ...
    try {
      await db.collection("communityPhotos").doc(id).delete();
      await renderCommunityGallery();
    } catch (err) {
      console.error(err);
    }
